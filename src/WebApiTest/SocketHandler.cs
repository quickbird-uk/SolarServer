...
namespace WebApiTest
{
    ...
    {
        private static ConcurrentDictionary<Guid, SocketHandler> _ActiveConnections = new ConcurrentDictionary<Guid, SocketHandler>(); 
        ...
        SocketHandler(WebSocket socket, Guid id)
        {
            this.socket = socket;
            this.id = id; 
        }
        ...
        */
        ...
        async Task SendMessageAsync(ArraySegment<byte> message)
        {
            try
            {
        ...
        public static async void MessageAll(string message)
        {
            ...
            ArraySegment<byte> rawBytes = new ArraySegment<byte>(messageInBytes);

            foreach (var connection in connections)
            {
                connection?.SendMessageAsync(rawBytes); 
            }
        }
        ...
        async Task RecieveLoop()
        {
            ...
            while (this.socket.State == WebSocketState.Open)
            {
                ...
                byte[] message = Encoding.UTF8.GetBytes("Revieced");
            ...
            _ActiveConnections.TryRemove(id, out socketHandler); 
        }
        ...
        static async Task Acceptor(HttpContext hc, Func<Task> n)
        {
            ...
            var socketHanler = new SocketHandler(socket, Guid.NewGuid());
            _ActiveConnections.TryAdd(socketHanler.id, socketHanler); 
            await socketHanler.RecieveLoop();
        }
